...
func asdeps(cmdArgs1 *settings.PacmanConf, rt *Runtime, pkgs []string) error {
...
func asexp(cmdArgs *settings.PacmanConf, rt *Runtime, pkgs []string) error {
	if len(pkgs) == 0 {
		return nil
	}
...
func install(rt *Runtime, pacmanConf *settings.PacmanConf, sconf *settings.SConf, ignoreProviders bool) (err error) {
	...
	if sconf.SysUpgrade != 0 {
		...
		ignore, aurUp, errUp := upgrade.UpgradePkgs(rt.Config, aurUp, repoUp)
		if errUp != nil {
			return errUp
		}
...
func removeMake(do *dep.Order, rt *Runtime) error {
...
func earlyPacmanCall(rt *Runtime, pacmanConf *settings.PacmanConf, sconf *settings.SConf) error {
...
func earlyRefresh(cmdArgs *settings.PacmanConf, rt *Runtime) error {
...
func cleanNumberMenu(bases []dep.Base, installed stringset.StringSet, hasClean bool, answerClean, buildDir string, noConfirm bool) ([]dep.Base, error) {
	...
	cleanInput, err := view.GetInput(answerClean, noConfirm)
	if err != nil {
		return nil, err
	}
...
func editDiffNumberMenu(bases []dep.Base, installed stringset.StringSet, diff bool, answerDiff, answerEdit string, noConfirm bool) ([]dep.Base, error) {
	...
	if diff {
		...
		text.Infoln(text.Tf("%s [A]ll [Ab]ort [I]nstalled [No]tInstalled or (1 2 3, 1-3, ^4)", text.Cyan(text.T("[N]one"))))
		editInput, err = view.GetInput(answerDiff, noConfirm)
		if err != nil {
			return nil, err
		}
	} else {
		...
		text.Infoln(text.Tf("%s [A]ll [Ab]ort [I]nstalled [No]tInstalled or (1 2 3, 1-3, ^4)", text.Cyan(text.T("[N]one"))))
		editInput, err = view.GetInput(answerEdit, noConfirm)
		if err != nil {
			return nil, err
		}
	}
...
func editPkgbuilds(bases []dep.Base, srcinfos map[string]*gosrc.Srcinfo, conf *settings.YayConfig) error {
	...
	if len(pkgbuilds) > 0 {
		editor, editorArgs := view.Editor(conf.Editor, conf.EditorFlags, conf.Pacman.NoConfirm)
