...
func cleanDependencies(rt *Runtime, cmdArgs *settings.PacmanConf, removeOptional bool) error {
	hanging := query.HangingPackages(removeOptional, rt.DB)
	if len(hanging) != 0 {
...
func cleanRemove(rt *Runtime, cmdArgs *settings.PacmanConf, pkgNames []string) error {
	if len(pkgNames) == 0 {
		return nil
	}
...
func cleanUntracked(rt *Runtime) error {
...
func cleanAfter(rt *Runtime, bases []dep.Base) {
	text.Println(text.T("removing untracked AUR files from cache..."))
